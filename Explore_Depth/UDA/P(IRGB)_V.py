...
MAIN_FOLDER_P = FOLDER + 'postdam/'
DATA_FOLDER_P = MAIN_FOLDER_P + 'img_rgbir/top_potsdam_{}_RGBIR.tif'
LABEL_FOLDER_P = MAIN_FOLDER_P + 'label/top_potsdam_{}_label.tif'
Depth_FOLDER_P = MAIN_FOLDER_P + 'depth_3000/top_potsdam_{}_RGB.png'

MAIN_FOLDER_V = FOLDER + 'vaihingen/'
DATA_FOLDER_V = MAIN_FOLDER_V + 'img_irrg/top_mosaic_09cm_area{}.tif'
LABEL_FOLDER_V = MAIN_FOLDER_V + 'label/top_mosaic_09cm_area{}.tif'
Depth_FOLDER_V = MAIN_FOLDER_V + 'depth_1281/top_mosaic_09cm_area{}.png'
...
def train(epochs, start_save_epoch, save_epoch, weights=WEIGHTS):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch >= start_save_epoch and epoch % save_epoch == 0:
            ...
            start_time = time.time()
            if MIoU > MIoU_best:
                torch.save(model.state_dict(), '/data5/GLGAN/result_xiugai/p(irgb)_v/CGA_baoliu_pred1/P(irgb)-V(IRGB)_epoch{}_{}'.format(epoch, MIoU))
                MIoU_best = MIoU
    print("Train Done!!")
...
# #####   test   ####
# model.load_state_dict(torch.load('/data5/GLGAN/result_xiugai/p(irgb)_v/yuanshi/P(irgb)-V_epoch45_0.6638867867219056'))
# acc, all_preds, all_gts = test(test_ids_V, all=True, stride=128)
# print("Acc: ", acc)

# for p, id_ in zip(all_preds, test_ids_V):
#    img = convert_to_color(p)
#    io.imsave('/data5/open_code/test/UDA/CCAGAN/yuanshi/PRGB2V_' + str(acc) + '_tile_{}.png'.format(id_), img)
